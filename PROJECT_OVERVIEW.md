...
To ensure reliability across distributed services, VendorIQ.ai uses the Saga pattern implemented using **Choreography** with a Kafka-based event-driven pipeline. When an invoice arrives—typically through email—Kafka handles the communication between the Email Service and the OCR Service. This ensures data consistency, fault tolerance, and guaranteed processing even when one service is temporarily down.
